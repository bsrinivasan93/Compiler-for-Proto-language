...
class Node:
    ...
    def checkForStaticSemantics(self, sib_list, param={}):
        global sym_deflist
        self.sib_list = sib_list
        self.param = param
        ...
        for i in range(len(self.children)):
            if isinstance(self.children[i] , self.__class__)==True:
                ret = self.children[i].checkForStaticSemantics(self.children[i+1:], self.param)
    ...
    def gencode(self):
        ...
        if self.type in ['stmt_se']:
            ...
            assert(len(node_icode_list) == 2)
            return node_icode_list[0]+ '['+ node_icode_list[1]+ ']'
