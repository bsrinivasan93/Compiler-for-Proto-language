...
class Node:
    ...
    def getFormalRetType(self):
        #print 'inside getFormalRetType:', self.sym_tab
        for i in range(len(self.sym_tab)-1, -1, -1):
            symbol = self.sym_tab[i][0]
            #print "inside getFormalRetType", symbol
            #sym_index, sym_dict = getSymTabDict(symbol)
    ...
    def checkStaticSemantics(self):
        ...
        if self.type in ['type_int']:
            ...
            if self.fun_flag == 0:
                sym_tab.append([None,{}])
            #print "symtab after entering block", sym_tab
        ...
        for i in range(len(self.children)):
            if isinstance(self.children[i] , self.__class__)==True:
                ...
                if self.type == 'vardecl':
                    ...
                    assert len(self.leaves) == 1
                    assert len(self.children[0].leaves) == 1
                    #print "inside fundecl : ", self.formalsOpt_id, self.formalsOpt_type, self.children[1].formalsOpt_id, self.children[1].formalsOpt_type
                    ...
                    #if self.children[1].type == 'formalsOpt':
                    ...
                    if len(self.formalsOpt_type) == 1 and self.formalsOpt_type[0] == None:
                        ...
                        for i in range(len(self.formalsOpt_type)):
                            #print "inside fundecl", self.formalsOpt_id[i], self.formalsOpt_type[i]
                            #For normal variables
                            ...
                            #print i, self.formalsOpt_id[i], temp_dict
                            temp_list = [self.formalsOpt_id[i], temp_dict]
                            sym_tab.append(temp_list)
                            #sym_tab.append([self.formalsOpt_id[i], {'type':'var_fargs', 'dtype':self.formalsOpt_type[i], 'sym_state':'INIT', 'sym_dimensions':[], 'sym_value':[], 'proc_attr':[]}])

                    self.children[2].fun_flag = 1


        #print "Node(", hex(id(self)), "):", self.type, self.children, self.leaves, self.dtype, sym_tab
        if self.type in ['type_array']:
            #print "inside type_array", self.dtype, self.leaves
            assert len(self.children) == 1
            #assert len(self.leaves) == 0
            self.dtype = self.children[0].dtype + '[]'
            #self.leaves.append(self.dtype)
            self.leaves = [self.children[0].dtype + '[]']

        elif self.type in ['stmt_block']:
            #print "symtab before exiting block", sym_tab
            sym_tab = symtab_backup

        elif self.type in ['formals_single']:
            #print "inside formals_single", self.children[0].leaves
            assert len(self.children) == 1
            assert len(self.leaves) == 1
            assert len(self.children[0].leaves) == 1
            ...
            #print "inside formals_multi", self.children[0].type, self.children[0].dtype, self.children[0].leaves
            assert len(self.children) == 2
            assert len(self.leaves) == 1
            assert len(self.children[0].leaves) == 1
            self.formalsOpt_id.append(self.leaves[0])
        ...
        '''
            ...
            assert  len(self.leaves) == 0
            assert len(self.children[0].leaves) == 1
            self.leaves = self.children[0].leaves
    ...
    def typeCheck(self):
        ...
        if self.type in ['pgm']:
            ...
            #print "ins stmt_if: ", self.children[0].dtype,
            assert len(self.children) == 2
            assert len(self.leaves) == 0
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            ...
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            #print 'inside stmt_return :', self.children[0].type, self.children[0].dtype, sym_tab
            ...
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            #print "inside SEOpt, AEOpt:", self.children[0].dtype
            self.dtype = self.children[0].dtype
            ...
            assert len(self.leaves) == 0
            assert self.dtype == ''
            #print "inside newArray", self.children[0].dtype, self.children[1].dtype, self.children[2].dtype
            assert self.children[0].dtype != ''
            assert self.children[1].dtype != ''
            #assert self.children[2].dtype != ''
            if self.children[1].dtype == 'int':
            ...
            #print "inside functionCall:", self.children, self.leaves, self.children[0].dtype
            assert len(self.children) == 1
            assert len(self.leaves) == 1
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            ...
            #Calculate formal arguments type list
            formal_arg_list = []
            farg_list = sym_dict['proc_attr'][1]
            #print "farg_list:", farg_list, farg_list != [None]
            if farg_list != [None]:
            ...
            #print farg_list, formal_arg_list
            ...
            #print formal_ret_type

            #Calculate actual arguments type list
            actual_arg_list = self.children[0].dtype
            #print formal_arg_list, actual_arg_list, formal_ret_type
            ...
            #print 'inside args_single', self.children[0].dtype
            assert len(self.children) == 1
            assert len(self.leaves) == 0
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            ...
            #print 'inside args_multi', self.children[0].dtype, self.children[1].dtype
            assert len(self.children) == 2
            assert len(self.leaves) == 0
            assert self.dtype == ''
            assert self.children[0].dtype != ''
            ...
            #print 'inside argsOpt', self.children[0].dtype
            assert len(self.children) == 1
            assert len(self.leaves) == 0
            assert self.dtype == ''
            assert self.children[0].dtype != ''
    ...
    def checkControlFlowReturn(self):
        ...
        if self.type in ['stmt_return']:
            ...
            #print "inside stmt_if_else", self.return_list
    ...
    def resolveArgs(self, symbol):
        #print "inside resolveArgs", symbol, self.sym_tab
        temp_dict = {}
        for i in range(len(self.sym_tab)-1,-1,-1):
            if self.sym_tab[i][0] == symbol:
                temp_dict = self.sym_tab[i][1]
                break
            #elif self.sym_tab[i][0] == None:
            #    print "error in resolveArgs"
            #    exit(1)
        #print "inside resolveArgs", symbol, temp_dict
    ...
    def gencode(self, ircode):
        ...
        #if self.type in ['pgm', 'declstar', 'declstar_empty', 'decl_']:
        #    return

        if self.type in ['fundecl']:
            #print "before: inside fundecl", hex(id(ircode)), self.leaves[0], ircode.func_name, ircode.ircode_list
            assert len(self.leaves) == 1
            if ircode.func_name != self.leaves[0]:
                return
            else:
                #Generate the intermed code for the function rooted here
                ircode.ircode_list = []
            ...
            copy_sym_tab = copy.deepcopy(sym_tab)
        ...
        if self.type in ['fundecl']:
            #print "after: inside fundecl", self.leaves[0], ircode.func_name, ircode.ircode_list
            #if ircode.func_name != self.leaves[0]:
            if ircode.func_name!='main':
            ...
            #print "inside stmt_return", node_icode_list, temp_dict
            ...
            '''
            if node_icode_list[0] not in sym_tab:
                sym_tab[node_icode_list[0]]=[node_icode_list[1]]
            else:
                sym_tab[node_icode_list[0]].append(node_icode_list[1])
            '''
            if self.children[0].type == 'lhs_id':
            ...
            assert(len(self.leaves) == 1)
            #print "\n\tinside lhs_id", self.leaves[0], self.sym_tab
            return self.resolveArgs(self.leaves[0])

        elif self.type in ['lhs_arrayAccess']:
            assert len(node_icode_list) == 1
            #print "inside lhs_arrayAccess", node_icode_list[0]
            ...
            assert len(node_icode_list) == 1
            #print "inside primary_arrayAccess", node_icode_list[0]
            temp_list = node_icode_list[0]
            temp1 = self.children[0].t
            temp2 = self.children[0].at
            size = 4
            ...
            assert(len(node_icode_list) == 2)
            #print "in arrayAccess:", node_icode_list
            temp1 = gen_temp()
            temp2 = gen_temp()
            self.t = temp1
            self.at = temp2
            ...
            assert len(node_icode_list) == 1
            #print "inside ae_primary", node_icode_list
            return node_icode_list[0]
            ...
            assert len(self.leaves) == 1
            #print "\n\tinside primary_id:", self.leaves[0], self.sym_tab
            temp = self.resolveArgs(self.leaves[0])
            #print "inside primary_id:", self.leaves[0], temp
            return temp
            ...
            #print "inside args_multi", temp_list
            return temp_list

        elif self.type in ['args_single']:
            assert node_icode_list != [None]
            #print "inside args_single", node_icode_list
            return node_icode_list


        elif self.type in ['newArray']:
            #print "\n\tinside ae_new", node_icode_list
