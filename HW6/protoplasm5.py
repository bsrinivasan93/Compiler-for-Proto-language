#Balaji Srinivasan      -   05/11/15
#protoplasm5.py         -   Program that compiles a Proto source program and generates a .asm file of the same name
#For grammar, refer to http://www3.cs.stonybrook.edu/~cram/cse504/Spring15/Homeworks/hw6.html
...
#Flags and patterns for optimizations
constant_propagation_flag = 0
copy_propagation_flag = 0
cs_elimination_flag = 0
loop_invariant_flag = 0
induction_var_flag = 0
constant_propagation_pattern = re.compile("--constantpropagation=on")
copy_propagation_pattern = re.compile("--copypropagation=on")
cs_elimination_pattern = re.compile("--cselimination=on")
loop_invariant_pattern = re.compile("--loopmotion=on")
induction_var_pattern = re.compile("--inductionvariables=on")
...
#Set the optimization flags from command-line-options
#print "command-line args", sys.argv
for i in sys.argv:
    if constant_propagation_pattern.match(i) is not None:
        constant_propagation_flag = 1
    elif copy_propagation_pattern.match(i) is not None:
        copy_propagation_flag = 1
    elif cs_elimination_pattern.match(i) is not None:
        cs_elimination_flag = 1
    elif loop_invariant_pattern.match(i) is not None:
        loop_invariant_flag = 1
    elif induction_var_pattern.match(i) is not None:
        induction_var_flag = 1
#print constant_propagation_flag, copy_propagation_flag, cs_elimination_flag, loop_invariant_flag, induction_var_flag
...
#Applying desired optimizations
for i in range(num_func):
    if constant_propagation_flag == 1:
        print "\n\tinside optimize_constant_propagation"
        func_ircode_list[i].optimize_constant_propagation()
'''
print "\n\tafter performing liveness analysis"
for i in range(num_func):
    func_ircode_list[i].performLivenessAnalysis()

print "\n\tafter finalizing gencode"
for i in range(num_func):
    func_ircode_list[i].finalize()
