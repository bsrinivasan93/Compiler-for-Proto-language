...
label_pattern = re.compile('label_[0-9]+|func_[A-Za-z][a-zA-Z0-9]*')
...
class IRCode:
    ...
    def getSuccHelper(self, index, res_list, rec_level):
        label_pattern = re.compile('label_[0-9]+')
        number_pattern = re.compile('[0-9]+')
        ...
        if index != len(self.ircode_list)-1 and self.ircode_list[index+1][0] == 'b':
            self.getSuccHelper(self.label_table[self.ircode_list[index+1][1]], res_list, rec_level+1)


        elif self.ircode_list[index][0] == 'b' or self.ircode_list[index][0] == 'beq':
            if self.ircode_list[index][0] == 'b' and label_pattern.match(self.ircode_list[index][1]) is not None:
                self.getSuccHelper(self.label_table[self.ircode_list[index][1]], res_list, rec_level+1)

            if self.ircode_list[index][0] == 'beq' and label_pattern.match(self.ircode_list[index][3]) is not None:
                self.getSuccHelper(self.label_table[self.ircode_list[index][3]], res_list, rec_level+1)
                self.getSuccHelper(index+1, res_list, rec_level+1)

        elif label_pattern.match(self.ircode_list[index][0]) is not None and self.ircode_list[index][1]==':':
            self.getSuccHelper(self.label_table[self.ircode_list[index][0]], res_list, rec_level+1)
        #    pass
    ...
    def getSucc(self, index):
        ...
        if index != len(self.ircode_list)-1:
            self.getSuccHelper(index, res_list, 0)
        else:
            res_list = [len(self.ircode_list)]
    ...
    #Generate predecessor list from successor list
    def gen_predlist(self):
        self.pred_list.append([])
        for i in range(len(self.succ_list)):
            temp_list = self.succ_list[i]
            for j in range(len(temp_list)):
                #print i, j,temp_list[j]
                self.pred_list[temp_list[j]].extend([i])


    def findType(self, ircode_list):
        if len(ircode_list) == 3:
            if ircode_list[1] == '=' :
                assert number_pattern.match(ircode_list[0]) is None
                if number_pattern.match(ircode_list[2]) is not None:
                    return 'assign_constant'
                elif identifier_pattern.match(ircode_list[2]) is not None:
                    return 'assign_id'
                elif ircode_list[2] == 'input':
                    return 'assign_input'

        elif len(ircode_list) == 4:
            if ircode_list[1] == '=' :
                assert number_pattern.match(ircode_list[0]) is None
                if identifier_pattern.match(ircode_list[3]) is not None:
                    return 'assign_unop'
            elif ircode_list[0] == 'beq':
                return 'stmt_beq'
            ...
            if ircode_list[1] == '=' :
                assert number_pattern.match(ircode_list[0]) is None
                if identifier_pattern.match(ircode_list[2]) is not None and identifier_pattern.match(ircode_list[4]) is not None:
                    return 'assign_binop'

        elif len(ircode_list) == 2:
            if label_pattern.match(ircode_list[0]) is not None:
                return 'stmt_label'
            elif ircode_list[0] == 'b' and label_pattern.match(ircode_list[1]) is not None:
                return 'stmt_b'



    def optimize_constant_propagation(self):
        ircode_copy = copy.deepcopy(self.ircode_list)

        for i in range(len(self.succ_list)):
            self.constant_prop_list.append({})
            print i,self.ircode_list[i], self.succ_list[i]

        self.gen_predlist()
        print len(self.pred_list), len(self.succ_list), self.constant_prop_list

        for i in range(len(self.succ_list)):
            print i,self.ircode_list[i], self.succ_list[i], self.pred_list[i]

        for i in range(len(ircode_copy)):
            if len(self.pred_list[i]) == 1:
                #print i, len(self.pred_list[i])
                self.constant_prop_list[i] = self.constant_prop_list[self.pred_list[i][0]]
            elif len(self.pred_list[i]) > 1:
                for j in range(len(self.pred_list[i])):
                    print "\n\t", i, j, self.constant_prop_list[self.pred_list[i][j]]
                    pass

            ircode_type = self.findType(ircode_copy[i])
            if ircode_type == 'assign_constant':
                #print i, ircode_copy[i], "assign-const"
                self.constant_prop_list[i][ircode_copy[i][0]] = ircode_copy[i][2]
            elif ircode_type == 'assign_id':
                temp_var = ircode_copy[i][2]
                if temp_var in self.constant_prop_list[i]:
                    self.ircode_list[i][2] = self.constant_prop_list[i][temp_var]
            elif ircode_type in ['assign_unop']:
                temp_var = ircode_copy[i][3]
                if temp_var in self.constant_prop_list[i]:
                    self.ircode_list[i][3] = self.constant_prop_list[i][temp_var]
                pass
            elif ircode_type in ['assign_binop']:
                temp_var1 = ircode_copy[i][2]
                temp_var2 = ircode_copy[i][4]
                if temp_var1 in self.constant_prop_list[i]:
                    self.ircode_list[i][2] = self.constant_prop_list[i][temp_var1]
                if temp_var2 in self.constant_prop_list[i]:
                    self.ircode_list[i][4] = self.constant_prop_list[i][temp_var2]
                if number_pattern.match(self.ircode_list[i][2]) is not None and number_pattern.match(self.ircode_list[i][4]) is not None:
                    pass
        for i in range(len(ircode_copy)):
            print i, ircode_copy[i], self.ircode_list[i]
