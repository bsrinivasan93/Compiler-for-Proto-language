...
def p_pgm(p):
    'pgm : stmt_seq'
    #print "matched p_pgm", [p[i] for i in range(len(p))]
    p[0] = Node('pgm', [p[1]], [])

def p_stmt_seq(p):
    '''stmt_seq : stmt stmt_seq
                | stmt'''
    #print "matched p_stmt_seq", [p[i] for i in range(len(p))]
    if len(p) == 3:
